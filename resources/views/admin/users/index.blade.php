    ...
    <div class="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50 to-indigo-100 py-8">
        <div class="max-w-7xl mx-auto px-4 space-y-8">
            ...
            <div class="text-center space-y-4 mb-8">
                <h2 class="text-4xl font-bold bg-gradient-to-r from-gray-800 to-gray-600 bg-clip-text text-transparent">Gestión de Usuarios</h2>
                <p class="text-gray-600 text-lg">Administra todos los usuarios del sistema</p>
                <div class="flex flex-col sm:flex-row justify-center items-center gap-4 mt-4">
                    <button id="viewToggle" class="inline-block bg-gray-600 text-white px-4 py-2 rounded-xl shadow hover:bg-gray-700 transition font-semibold">
                        <i class="fas fa-table mr-2"></i>Vista Tabla
                    </button>
                    <a href="{{ route('admin.users.create') }}" class="inline-block bg-gradient-to-r from-blue-600 to-purple-600 text-white px-6 py-2 rounded-xl shadow hover:from-blue-700 hover:to-purple-700 transition font-semibold">
            ...
            <!-- Filtros y Búsqueda -->
            <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg shadow-gray-200/50 border border-white/20 p-6 mb-6">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div class="flex items-end">
                        <button id="clearFilters" class="w-full bg-gray-500 text-white px-4 py-2 rounded-lg hover:bg-gray-600 transition">
                ...
                <div class="bg-green-50/80 backdrop-blur-sm border border-green-200 rounded-2xl p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-bold text-green-800">{{ $users->where('status', 'active')->count() ?: $users->count() }}</p>
                        </div>
                ...
                <div class="bg-yellow-50/80 backdrop-blur-sm border border-yellow-200 rounded-2xl p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-bold text-yellow-800">{{ $users->where('role', 'admin')->count() }}</p>
                        </div>
            ...
            <!-- Grid de Usuarios -->
            <div id="usersGrid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6">
                @forelse($users as $user)
                <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 border border-gray-100 user-card"
                    ...
                    <div class="border-t border-gray-100 p-4">
                        <div class="flex flex-wrap gap-2">
                            <a href="{{ route('admin.users.show', $user->id) }}"
                                class="flex-1 bg-gradient-to-r from-blue-500 to-purple-600 text-white px-3 py-1.5 rounded-xl text-center hover:from-blue-600 hover:to-purple-700 transition text-xs font-medium">
                                <i class="fas fa-eye mr-1"></i>Ver
                            </a>
                            <a href="{{ route('admin.users.edit', $user->id) }}"
                                class="flex-1 bg-gradient-to-r from-yellow-500 to-yellow-600 text-white px-3 py-1.5 rounded-xl text-center hover:from-yellow-600 hover:to-yellow-700 transition text-xs font-medium">
                                <i class="fas fa-edit mr-1"></i>Editar
                            </a>
                        </div>
                        <div class="flex flex-wrap gap-2 mt-2">
                            <a href="{{ route('admin.users.roles', $user->id) }}"
                                class="flex-1 bg-gradient-to-r from-green-500 to-green-600 text-white px-3 py-1.5 rounded-xl text-center hover:from-green-600 hover:to-green-700 transition text-xs font-medium">
                                <i class="fas fa-user-cog mr-1"></i>Roles
                            </a>
                            <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST"
                                onsubmit="return confirm('¿Estás seguro de eliminar este usuario?');" class="flex-1">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                    class="w-full bg-gradient-to-r from-red-500 to-red-600 text-white px-3 py-1.5 rounded-xl hover:from-red-600 hover:to-red-700 transition text-xs font-medium">
            ...
            <div id="usersTable" class="hidden bg-white/80 backdrop-blur-sm rounded-2xl shadow overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="py-3 px-6 text-sm text-gray-900">
                                ...
                                <div class="space-y-1">
                                    ...
                                    <div class="text-xs text-blue-600">+{{ $user->equipment->count() - 2 }} más</div>
                                    @endif
                                </div>
                                @else
                            ...
                            <td class="py-3 px-6">
                                <div class="flex flex-wrap gap-2">
                                    <a href="{{ route('admin.users.show', $user->id) }}" class="bg-gradient-to-r from-blue-500 to-purple-600 text-white px-3 py-1 rounded-xl hover:from-blue-600 hover:to-purple-700 transition text-xs font-semibold">Detalles</a>
                                    <a href="{{ route('admin.users.edit', $user->id) }}" class="bg-gradient-to-r from-yellow-500 to-yellow-600 text-white px-3 py-1 rounded-xl hover:from-yellow-600 hover:to-yellow-700 transition text-xs font-semibold">Editar</a>
                                    <a href="{{ route('admin.users.roles', $user->id) }}" class="bg-gradient-to-r from-green-500 to-green-600 text-white px-3 py-1 rounded-xl hover:from-green-600 hover:to-green-700 transition text-xs font-semibold">Roles</a>
                                    <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST" onsubmit="return confirm('¿Estás seguro?');" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="bg-gradient-to-r from-red-500 to-red-600 text-white px-3 py-1 rounded-xl hover:from-red-600 hover:to-red-700 transition text-xs font-semibold">Eliminar</button>
                                    </form>
                                </div>
                            </td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="6" class="py-6 px-6 text-center text-gray-500">No hay usuarios registrados.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
